...
impl AdsRuntime {
        ...
        O: serde::de::DeserializeOwned,
    {
        ...
        let (cache, entry) = self.resolve_symbol(fb_path)?;
        ...
        let handle = match cached_handle {
            Some(handle) => handle,
            None => {
                let handle = self.get_handle_by_name(format!("{fb_path}#{}", method.name()))?;
                entry.write()?.set_method_handle(method.name(), handle);
                handle
            }
        };
