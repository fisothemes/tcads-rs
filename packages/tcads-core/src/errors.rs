...
pub enum AdsError {
    ...
    ParseNetId(#[from] ParseNetIdError),
    #[error("Invalid Address: {0}")]
    ParseAmsAddr(#[from] ParseAmsAddrError),
    /// ADS return codes.
    #[error("ADS Device Error: {0}")]
    Ads(#[from] AdsReturnCode),
}

/// Error returned when parsing an AmsNetId string fails.
...
pub enum ParseAmsAddrError {
    #[error("Invalid format: expected 'NetId:Port'")]
    Format,
    #[error("Invalid NetID: {0}")]
    NetId(#[from] ParseNetIdError),
    #[error("Invalid Port: {0}")]
    Port(#[from] std::num::ParseIntError),
}

/// The official ADS Return Codes
/// Reference: Beckhoff ADS Specification (TE1000)
...
mod tests {
    use super::*;
    #[test]
    fn test_return_code_to_u32() {
        let return_code: u32 = AdsReturnCode::AdsErrDeviceError.into();
        assert_eq!(return_code, 0x700);
    }

    #[test]
    fn test_u32_to_return_code() {
        let return_code: AdsReturnCode = 0x502.into();
        assert_eq!(return_code, AdsReturnCode::RouterErrMailboxFull);
    }
}
