...
impl AdsSymbolUploadInfo {
    ...
    pub fn try_from_slice(data: &[u8]) -> Result<Self, AdsSymbolUploadInfoError> {
        ...
        Ok(Self {
            ...
            symbol_blob_size: u32::from_le_bytes(data[4..8].try_into().unwrap()),
            data_type_count: u32::from_le_bytes(data[8..12].try_into().unwrap()),
            data_type_blob_size: u32::from_le_bytes(data[12..16].try_into().unwrap()),
...
mod tests {
    ...
    fn parses_real_capture() {
        ...
        assert_eq!(info.data_type_blob_size(), 11296);
